function    intergrowth(Rate,Balance,Invest_N,Invest_V,a_1,a_2,a_3,b_1,b_2,b_3,NVratio_max,NVratio_best,NVratio_min,V_boundary,maxn,Invest_time,Max_car,figure_n)
N_s         = zeros(maxn,1);
V_s         = zeros(maxn,1);
for n = 1:maxn-1
    ...
    V_s(n+1)        = (b_1*Invest_V*(n<Invest_time) - a_3*V_s(n)*interinfluence + praise - b_3*abs(Rate - Balance)) * (1-V_s(n)/Max_car) + V_s(n);
    if N_s(n+1) < 0
        N_s(n+1) = 0;
    end
    if V_s(n+1) < 0
